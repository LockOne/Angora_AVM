...
impl<'a> AvmSearch<'a> {
    ...
    fn compute_delta_all(input: &mut MutInput, grad: &Grad, step: usize, sign : bool) {
        let step = step as f64;
        for (i, g) in grad.iter().enumerate() {
            let movement = g.pct * step;
            input.update(i, !(g.sign ^ sign), movement as u64);
        }
    }
    ...
    fn avm(
        ...
        let mut guess_step = if vsum > 0 {
            let guess_step = f0 / vsum;
            debug!(
                "f0 is : {}, vsum: {}, input: {:?}, guess step is : {}",
                f0, vsum, input, guess_step
            );
            Self::compute_delta_all(&mut input, grad, guess_step as usize, true);
        ...
        //first loop to get l and r value
        //check
        Self::compute_delta_all(&mut input, grad, guess_step as usize, true);
        let f_plus = self.execute(&input);
        Self::compute_delta_all(&mut input, grad, (guess_step * 2) as usize, false);
        let f_minus = self.execute(&input);

        Self::compute_delta_all(&mut input, grad, guess_step as usize, true);

        if f_minus >= f_last && f_plus >= f_last {
            return f_last;
        }

        let orig_step = guess_step as i64;
        guess_step *= 2;
        let search_sign = if f_minus < f_plus { f_last = f_minus; false } else { f_last = f_plus; true };
        ...
        loop {
            ...
            Self::compute_delta_all(&mut input, grad, guess_step as usize, search_sign);
            let f_new = self.execute(&input);
        ...
        let (mut l,mut r) = if search_sign {
            (-((guess_step / 2) as i64), guess_step as i64)
        } else {
            (-(guess_step as i64), (guess_step / 2) as i64)
        };
        ...
        while l < r {
            ...
            Self::compute_delta_all(&mut input, grad, ((l + r) / 2).abs() as usize, ((l + r) / 2) > 0);
            let f_one = self.execute(&input);
            Self::compute_delta_all(&mut input, grad, orig_step as usize, true);
            let f_two = self.execute(&input);
        ...
        trace!("update input: f0= {}, f_last={}, step={}", f0, f_last, orig_step);
