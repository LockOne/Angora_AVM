...
impl<'a> FnFuzz<'a> {
    ...
    pub fn run(&mut self) {
        //len, cond.variables.len, input_len, input_len == len, mien_len = min(len, output.len)
        ...
        if self.handler.cond.variables.len() < len { error!("variable length is less than input length. cond : {:?}",self.handler.cond); return; }
