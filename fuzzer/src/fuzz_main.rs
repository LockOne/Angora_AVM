...
pub fn fuzz_main(
    ...
    record_parameters(&angora_out_dir, &command_option, in_dir);
...
fn record_parameters(out_dir : &PathBuf, command: &command::CommandOpt, in_dir : &str){
   let fuzz_param = out_dir.join("parameters");
   let mut buff = match fs::File::create(&fuzz_param) {
     Ok(a) => a, Err(e) => {error!("Could not create file : {:?}", e); panic!();}
   }; 
    write!(buff, "out_dir : {}, in_dir : {}, mem_limit : {}, subject : {}, timeout : {}H",
                 out_dir.to_str().unwrap(), in_dir, command.mem_limit, command.main.0,
                 config::TOTAL_TIMEOUT / 3600).expect("Could not write to param file");
    writeln!(buff, "").expect("Could not write to param file");
    for s in &command.main.1 {
      write!(buff, " {}", s).expect("Could not write to param file");
    }
    writeln!(buff, "").expect("Could not write to param file");
    write!(buff, ", MAP_SIZE : {}", config::MAP_SIZE_POW2).expect("Could not write to param file");
}
...
fn main_thread_sync_and_log(
    ...
    while running.load(Ordering::SeqCst) {
        ...
        if init_time.elapsed() >= Duration::from_secs(config::TOTAL_TIMEOUT.into()) { running.store(false, Ordering::SeqCst);}
        sync_counter -= 1;
